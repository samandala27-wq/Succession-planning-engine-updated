...
EMP_COL_MAP = {
    ...
    "Grade":                          "_Grade_Raw",          # intermediate — converted below
    ...
    "Mobility_Preference":            "_Mobility_Raw",       # intermediate — converted below
...
def normalise_employees(df_raw):
    ...
    kf_blend_cols = [c for c in df.columns if c.startswith("KF_") and not c.startswith("KF_Risk")]
    if kf_blend_cols and "KF Blended Assessment Composite (1-5)" not in df.columns:
        raw_mean = df[kf_blend_cols].mean(axis=1)
        raw_max  = df[kf_blend_cols].max().max()
        df["KF Blended Assessment Composite (1-5)"] = (raw_mean / raw_max * 4 + 1).round(2)

    # Individual KFALP sub-scores mapped from KF columns we have
    kf_submap = {
        "KF KFALP - Drivers Score (1-5)":       "KF_Drivers__Achievement_Drive",
        "KF KFALP - Curiosity Score (1-5)":      "KF_Learning_Agility__Intellectual_Curiosity",
        "KF KFALP - Insight Score (1-5)":        "KF_Individual_Success_Profile__Strategic_Agility",
        "KF KFALP - Engagement Score (1-5)":     "KF_People_Leadership__Team_Motivation_Engagement",
        "KF KFALP - Determination Score (1-5)":  "KF_Individual_Success_Profile__Persistence",
        "KF KFALP - Learnability Score (1-5)":   "KF_Learning_Agility__Seeking_New_Challenges",
        "KF viaEdge - Mental Agility Score (1-5)":   "KF_Agility__Mental_Agility",
        "KF viaEdge - People Agility Score (1-5)":   "KF_Agility__People_Agility",
        "KF viaEdge - Change Agility Score (1-5)":   "KF_Agility__Change_Agility",
        "KF viaEdge - Results Agility Score (1-5)":  "KF_Agility__Results_Agility",
        "KF viaEdge - Self-Awareness Score (1-5)":   "KF_Agility__Self_Awareness_Agility",
    }
    for target, source in kf_submap.items():
        if target not in df.columns and source in df.columns:
            # Source is on 1-10 scale → remap to 1-5
            df[target] = (df[source] / 10.0 * 4.0 + 1.0).round(2)
